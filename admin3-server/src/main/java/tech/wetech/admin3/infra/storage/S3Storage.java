...
public class S3Storage implements Storage {
  ...
  public void store(InputStream inputStream, long contentLength, String contentType, String keyName) {
    ...
    // 如果配置了storagePath，则加上路径前缀
    String objectKey = getObjectKey(keyName);
    putObject(bucketName, objectKey, inputStream, contentType, contentLength);
  }
  ...
  public InputStream getFileContent(String filename) {
    ...
    try (S3Client s3Client = getS3Client()) {
      return s3Client.getObject(req -> req.bucket(bucketName).key(objectKey));
    }
  }
  ...
  public String getUrl(String filename) {
    if (config.getAddress() != null && !config.getAddress().isEmpty()) {
      // 如果配置了storagePath，需要加上路径前缀
      String objectKey = getObjectKey(filename);
      return config.getAddress() + objectKey;
    }
    String bucketName = config.getBucketName();
    String objectKey = getObjectKey(filename);
    try (S3Client s3Client = getS3Client()) {
      return s3Client.utilities()
        .getUrl(req -> req.bucket(bucketName).key(objectKey)).toString();
    }
  }
