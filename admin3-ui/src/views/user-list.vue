              ...
              <el-table-column label="头像" align="center">
                <template #default="scope">
                  <el-image
                    ...
                    :src="getFullUrl(scope.row.avatar)"
                    :z-index="10"
                    :preview-src-list="[getFullUrl(scope.row.avatar)]"
                    preview-teleported
                  >
                  </el-image>
                </template>
              ...
              <el-table-column label="操作" width="200" fixed="right" align="center">
                <template #default="scope">
                  <div style="display: flex; align-items: center; justify-content: center; gap: 4px;">
                    <el-button type="primary" link :icon="Edit" @click="handleEdit(scope.$index, scope.row)" v-action:user:update>
                      编辑
                    </el-button>
                    <el-dropdown trigger="click" v-action:user:update>
                      <el-button type="primary" link>
                        更多<el-icon class="el-icon--right"><ArrowDown /></el-icon>
                      </el-button>
                      <template #dropdown>
                        <el-dropdown-menu>
                          <el-dropdown-item v-if="scope.row.state==='NORMAL'" @click="handleDisable(scope.row)">
                            <el-icon><Lock /></el-icon>禁用
                          </el-dropdown-item>
                          <el-dropdown-item v-if="scope.row.state==='LOCKED'" @click="handleEnable(scope.row)">
                            <el-icon><Unlock /></el-icon>启用
                          </el-dropdown-item>
                          <el-dropdown-item @click="handleResetPassword(scope.row)">
                            <el-icon><Key /></el-icon>重置密码
                          </el-dropdown-item>
                          <el-dropdown-item divided @click="handleDelete(scope.row)" v-action:user:delete>
...
import {Delete, Edit, Lock, Plus, Search, Unlock, Key, ArrowDown} from '@element-plus/icons-vue';
import {createUser, deleteUser, disableUser, enableUser, updateUser, resetPassword} from "../api/user";
import {
  createOrganization,
  deleteOrganization,
  getOrganizationTree,
...
const handleDisable = (row: any) => {
  ElMessageBox.confirm('确定要禁用该用户吗？', '提示', { type: 'warning' }).then(() => {
...
const handleEnable = (row: any) => {
  ElMessageBox.confirm('确定要启用该用户吗？', '提示', { type: 'warning' }).then(() => {
