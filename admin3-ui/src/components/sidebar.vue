...
getMenus().then(res => {
  ...
  // 找到"小程序管理"菜单并默认展开
  const miniappMenu = items.value.find(item => item.title === '小程序管理');
  if (miniappMenu) {
    defaultOpeneds.value = [miniappMenu.index];
  }
  menuReady.value = true;
  // 使用 nextTick 确保菜单渲染后手动打开
  nextTick(() => {
    if (menuRef.value && miniappMenu) {
      menuRef.value.open(miniappMenu.index);
    }
  });
});
